...
const Feedback = () => {
    ...
    return (
        <div className="py-5 sm:py-10 px-4 sm:px-8 md:px-20 bg-[#f4f7fa] dark:bg-[#1a202c] transition-colors duration-300">
            ...
            <div className="flex justify-center gap-6  overflow-x-auto sm:flex flex-wrap">
                {feedback.map(({ image, icon, name, company, comment }, index) => (
                    <div
                        key={index}
                        className="flex flex-col gap-5 border-2 border-[#0e1c30] rounded-lg p-6 sm:p-8 cursor-pointer transition-all duration-300 hover:shadow-lg w-full sm:w-[300px] md:w-[350px] dark:border-[#4a5568] dark:bg-[#2d3748] dark:text-white"
                    >
                        {/* User Info */}
